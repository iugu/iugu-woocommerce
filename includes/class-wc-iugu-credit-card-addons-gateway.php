...
class WC_Iugu_Credit_Card_Addons_Gateway extends WC_Iugu_Credit_Card_Gateway {
	...
	protected function process_subscription( $order_id ) {
		try {
			...
			// Nas subscriptions, o cartão é sempre salvo, independente da vontade do usuário, para podermos fazer as próximas cobranças.
			...
			if ( isset( $payment_response ) && is_wp_error( $payment_response ) ) {
				// Se a chamada foi mal sucedida, então a forma de pagamento criada será excluída.
				$this->api->remove_payment_method($order, $payment_method_id);
